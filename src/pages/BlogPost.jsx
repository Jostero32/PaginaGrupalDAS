  ...
  return (
    <article className="page-section">
      <div className="container max-w-[820px]">
        ...
        <div className="mt-6 flex flex-wrap items-center gap-3 text-[0.95rem] text-[rgba(57,62,65,0.78)]">
          <Badge variant="primary">{post.category}</Badge>
          <span className="font-semibold text-[var(--color-primary)]">{formatDate(post.date)}</span>
        ...
        <div className="flex items-center gap-3 mb-6 text-sm">
          <div
            ...
            style={{ background: "rgba(63, 136, 197, 0.16)", color: "var(--color-primary)" }}
          >
            {post.author.charAt(0)}
          </div>
          <div>
            <p className="m-0 font-semibold">{post.author}</p>
            <p className="m-0 muted">{post.role}</p>
          </div>
        </div>
        ...
        {post.keyTakeaways?.length ? (
          <div className="mt-8 p-5 rounded-[var(--radius-md)] border border-[rgba(57,62,65,0.1)] bg-white shadow-[0_10px_24px_rgba(57,62,65,0.06)]">
            <h3 className="m-0 mb-3 text-[1.1rem] text-[var(--color-text)]">Claves rapidas</h3>
